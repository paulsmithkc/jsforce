...
function zeropad(n: number) { return (n<10 ? "0" : "") + n; }
...
export class SfDate { 
  ...
  /**
   * Convert JavaScript date object to ISO8601 Date format 
   * (e.g. 2012-10-31)
   */
  static toDateLiteral(date: string | number | Date): SfDate {
    ...
    if (isNumber(date)) {
      ...
      parsedDate = SfDate.parseDate(date);
    } else {
      parsedDate = date;
    }
 ...
 */
export const SfDateLiterals = Object.freeze({
  YESTERDAY: 1,
  TODAY: 1,
  TOMORROW: 1,
  LAST_WEEK: 1,
